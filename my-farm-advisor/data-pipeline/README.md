...
## Integrated Field Intelligence Dashboard

The data-pipeline includes a reusable dashboard skill that generates a self-contained, offline HTML dashboard comparing all three demonstration growers (Illinois, Iowa, Nebraska) across field characteristics, weather, crop distribution, soil drainage, and vegetation health.
...
# Step 3 — Prepare summary datasets (Phase A)
"${DATA_PIPELINE_DATA_ROOT}/data-pipeline/.venv/bin/python" \
  scripts/reporting/prepare_dashboard_data.py

# Step 4 — Generate the dashboard (Phase B)
"${DATA_PIPELINE_DATA_ROOT}/data-pipeline/.venv/bin/python" \
...
```
${DATA_PIPELINE_DATA_ROOT}/data-pipeline/shared/dashboard/integrated_field_intelligence_dashboard.html
```

### Dashboard Features

- **Six tabs**: Overview, Field Locations, Weather, Crop Distribution, Vegetation Health, Interpretation
- **11 interactive Plotly charts** including field map, size distributions, temperature/precipitation cycles, crop composition, rotation heatmaps, NDVI time series, and GDD progression
- **Offline**: Zero CDN dependencies — Plotly.js is inlined at build time
- **Grower comparison**: All three growers across Illinois, Iowa, and Nebraska
- **Soil health metric**: SSURGO drainage class distribution with average pH and organic matter
- **KPI summary**: Overview tab with stat cards and per-grower comparison table
- **Interpretation**: Evidence-based observations and comprehensive limitations disclosure
...
"${DATA_PIPELINE_DATA_ROOT}/data-pipeline/.venv/bin/python" \
  scripts/reporting/generate_final_dashboard.py \
  --output ~/Desktop/my_dashboard.html
```

### Summary Datasets

The dashboard reads seven pre-computed summary CSV files from `shared/dashboard/`. These are generated by `prepare_dashboard_data.py` from the pipeline runtime:

| File | Description |
|------|-------------|
| `dashboard_field_summary.csv` | Per-field acreage, compactness, centroid coordinates |
| `dashboard_weather_summary.csv` | Monthly mean temperature and precipitation per grower |
| `dashboard_crop_summary.csv` | CDL crop composition percentages per grower |
| `dashboard_rotation_summary.csv` | Crop rotation transition probabilities |
| `dashboard_vegetation_summary.csv` | Sentinel-2 NDVI observations per field-date |
| `dashboard_data_coverage.csv` | Per-grower data availability summary |
| `dashboard_soil_summary.csv` | SSURGO drainage class, pH, and organic matter per field |

### Dependencies

- Python 3.10+
- pandas, numpy, geopandas (pipeline runtime venv)
- plotly >= 5.0 (for figure generation at build time)
- requests (for Plotly.js bundle download)
- Plotly.js v2.27.0 (downloaded and inlined at build time; no CDN dependency at runtime)

### Documentation

- [Supplementary Dashboard Info](docs/DASHBOARD_INFO.md) — project overview, dataset description, dashboard explanation, analytical interpretation, and limitations
- [AI Usage Documentation](docs/AI_USAGE.md) — AI tools, purpose, and verification steps
